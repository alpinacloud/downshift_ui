...
const Shape = styled.button<Pick<Props, 'size' | 'disabled'> & {isChecked: boolean}>(props => ({
  filter: `drop-shadow(${props.theme.shadow.sm})`,
  display: 'flex',
  position: 'relative',
  borderStyle: 'solid',
  borderWidth: 1,
  borderColor: props.disabled ? props.theme.color.disabled: props.isChecked ? props.theme.color.primary : props.theme.color.contrast,
  transition: `box-shadow ${props.theme.speed}s, border-color ${props.theme.speed}s, padding ${props.theme.speed}s, background-color ${props.theme.speed}s`,
  backgroundColor: props.isChecked
      ? props.disabled ? props.theme.color.disabled : props.theme.color.primary
      : props.disabled ? props.theme.color.disabledText : props.theme.color.surface,
  borderRadius: props.theme.space[props.size] * 3,
  padding: props.theme.space[props.size] / 2.5,
  paddingRight: props.isChecked ? props.theme.space[props.size] / 2.5 : props.theme.space[props.size] * 1.75,
  paddingLeft: !props.isChecked ? props.theme.space[props.size] / 2.5 : props.theme.space[props.size] * 1.75,
...
const StateIndicator = styled.span<Pick<Props, 'size' | 'disabled'> & {isChecked: boolean}>(props => ({
  display: 'inline-block',
  backgroundColor: props.isChecked
      ? props.disabled ? props.theme.color.disabledText : props.theme.color.primaryText
      : props.disabled ? props.theme.color.disabled : props.theme.color.primary,
  borderRadius: '100%',
